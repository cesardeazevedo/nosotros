...
describe('NoteStore', () => {
  test('assert root note and replies', () => {
    ...
    const eventReply = fakeNote({ id: '2', tags: [['e', '1']] })
    const eventReply2 = fakeNote({ id: '3', tags: [['e', '1']] })
    const eventReply3 = fakeNote({ id: '4', tags: [['e', '3']] })
  ...
  test('assert root note and with `a` tag', () => {
    ...
    const event2 = fakeNote({ id: '2', tags: [['a', '30023:1:key']] })
    const note1 = store.add(event1, parseNote(event1))
