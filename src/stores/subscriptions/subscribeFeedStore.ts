...
import { subscribeFeedListSetsE, subscribeFeedListSetsP } from '@/nostr/subscriptions/subscribeFeedListSets'
...
function subscribeFeedScope(feed: FeedStore, ctx: NostrContext, pagination: PaginationSubject) {
  ...
  switch (scope) {
    ...
    case 'sets_p': {
      return subscribeFeedListSetsP(pagination, ctx, options)
    }
    case 'sets_e': {
      return subscribeFeedListSetsE(pagination, ctx)
    }
    case 'inbox': {
